...
function vistaPrevia() {
  ...
  const numFactura = $("#input-num-factura").val().trim();
  ...
  if (!id_cliente.trim().length) {
  ...
  } else if (!numFactura || !numFactura.length || parseInt(numFactura) < 1) {
    swal("Ingresa el Número de la Factura que deseas anular", "", "error");
  } else {
    ...
    if (productosCotizados.length) {
      ...
      swal("Debes agregar algún producto a la Nota de Crédito!", "", "error");
    }
  }
}
        ...
        if ($("#table-pedido > tbody > tr").length < 1) {
          $("#table-pedido > tbody").append(`
              <tr class="pedido-vacio-msg">
                <th scope="row" colspan="8" class="text-center"><span class="text-muted">Agrega Productos a la Nota de Crédito</span></th>
              </tr>
            `);
        }
...
async function printCotizacion(dataCotizacion) {
              ...
              <div class="row mt-4 p-0">
                ...
                <div class="col-md-4">
                    <div style="border: 3px solid #FE9A2E !important;border-radius:10px; text-align: center;padding:5px;color:#F7BE81 !important;">
                        ...
                        <h5 style="color:#F7BE81 !important;">Nota de Crédito</h5>
  ...
  getFoliosDisponibles(null, null, 61, numFactura, null);
...
function anularFactura(rowid, folioRef, folioNC, cafNC) {
  ...
  if (hasChanged) {
    ...
    swal("No hay Folio para la Nota de Crédito", "", "error");
  } else {
    if (productosCotizados.length) {
      setLoading(true)
      ...
      $.ajax({
        ...
        type: "POST",
        data: {
          consulta: "anular_factura_antigua",
          id_cliente: id_cliente,
          jsonarray: jsonarray,
        ...
        success: function (x) {
          ...
          if (x.includes("path")) {
            ...
            window
              .open(
                `verpdf.php?tipo=NC&folio=${folioNC}&file=${data.path}`,
                "_blank"
              );
            setLoading(false)
            $("#modal-vistaprevia").modal("hide");
            ...
            ClearPedido();
            $("#modal-vistaprevia").modal("hide");
            swal("La Nota de Crédito SE ENVIÓ AL SII, pero hubo un error al actualizar los datos en LA BD.", "POR FAVOR COMUNICATE CON SOPORTE ANTES DE CONTINUAR. "+x, "error");
          }
          else if (x.includes("ERROR_ENVIO_SII")){
            setLoading(false)
            ClearPedido();
            $("#modal-vistaprevia").modal("hide");
            swal("La Nota de Crédito se guardó en el Sistema, pero hubo un error al enviarla al SII.", "Deberás buscar la Nota en el Historial e intentar reenviarla.", "error");
          } else {
