...
function vistaPrevia() {
  ...
  if (!id_cliente.trim().length) {
    ...
    if (productosCotizados.length) {
      ...
      swal("Debes agregar algún producto a la Boleta!", "", "error");
    }
  }
}
        ...
        if ($("#table-pedido > tbody > tr").length < 1) {
          $("#table-pedido > tbody").append(`
              <tr class="pedido-vacio-msg">
                <th scope="row" colspan="8" class="text-center"><span class="text-muted">Agrega Productos a la Boleta</span></th>
              </tr>
            `);
        }
              ...
              <div class="row mt-4 p-0">
                ...
                <div class="col-md-4">
                    <div style="border: 3px solid #FE9A2E !important;border-radius:10px; text-align: center;padding:5px;color:#F7BE81 !important;">
                        ...
                        <h5 style="color:#F7BE81 !important;">Boleta</h5>
...
function generarBoleta(id_cotizacion, id_guia, folioBOL, caf) {
  const id_cliente = $("#select_cliente").find("option:selected").val();
  ...
  if (hasChanged) {
    ...
    swal("No hay Folios para la Boleta", "", "error");
  } else {
    if (productosCotizados.length) {
      ...
      $.ajax({
        ...
        type: "POST",
        data: {
          consulta: "generar_boleta_directa",
          id_cliente: id_cliente,
          jsonarray: jsonarray,
        ...
        success: function (x) {
          ...
          if (x.includes("path")) {
            ...
            window
              .open(
                `verpdf.php?tipo=BOL&folio=${folioBOL}&file=${data.path}`,
                "_blank"
              );
            setLoading(false)
        ...
        error: function (jqXHR, estado, error) {
          swal(
            "Ocurrió un error al guardar la Boleta",
            error.toString(),
            "error"
          );
          setLoading(false)
