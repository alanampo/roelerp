...
function vistaPrevia() {
  ...
  if (!id_cliente.trim().length) {
    ...
    if (productosCotizados.length) {
      ...
      swal("Debes agregar algún producto a la Factura!", "", "error");
    }
  }
}
        ...
        if ($("#table-pedido > tbody > tr").length < 1) {
          $("#table-pedido > tbody").append(`
              <tr class="pedido-vacio-msg">
                <th scope="row" colspan="8" class="text-center"><span class="text-muted">Agrega Productos a la Factura</span></th>
              </tr>
            `);
        }
...
async function printCotizacion(dataCotizacion) {
              ...
              <div class="row mt-4 p-0">
                ...
                <div class="col-md-4">
                    <div style="border: 3px solid #FE9A2E !important;border-radius:10px; text-align: center;padding:5px;color:#F7BE81 !important;">
                        ...
                        <h5 style="color:#F7BE81 !important;">Factura</h5>
  ...
  getFoliosDisponibles(null, null, 33, null, null);
...
function generarFactura(id_cotizacion, id_guia, folioFACT, caf) {
  const id_cliente = $("#select_cliente").find("option:selected").val();
  ...
  if (hasChanged) {
    ...
    swal("No hay Folios para la Factura", "", "error");
  } else {
    if (productosCotizados.length) {
      ...
      $.ajax({
        ...
        type: "POST",
        data: {
          consulta: "generar_factura_directa",
          id_cliente: id_cliente,
          jsonarray: jsonarray,
        ...
        success: function (x) {
          ...
          if (x.includes("path")) {
            ...
            window
              .open(
                `verpdf.php?tipo=FACT&folio=${folioFACT}&file=${data.path}`,
                "_blank"
              );
            setLoading(false)
            ...
            ClearPedido();
            $("#modal-vistaprevia").modal("hide");
            swal("La Factura SE ENVIÓ AL SII, pero hubo un error al actualizar los datos en LA BD.", "POR FAVOR COMUNICATE CON SOPORTE ANTES DE CONTINUAR"+x, "error");
          }
          else if (x.includes("ERROR_ENVIO_SII")){
            setLoading(false)
            $("#modal-vistaprevia").modal("hide");
            ClearPedido();
            swal("La Factura se guardó en el Sistema, pero hubo un error al enviarla al SII.", "Deberás buscar la Factura en el Historial e intentar reenviarla.", "error");
          } else {
