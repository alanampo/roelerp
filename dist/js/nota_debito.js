...
$(document).ready(function () {
  ...
  $("#input-cantidad,#input-descuento,#input-num-factura").on(
...
function vistaPrevia() {
  ...
  //const numNC = $("#input-num-factura").val().trim();
  const numNC = $("#select-nc option:selected").attr("x-folio");
  const rowidNC = $("#select-nc option:selected").val().trim();
  ...
  if (!id_cliente.trim().length) {
  ...
  } else if (!rowidNC || !rowidNC.length || parseInt(rowidNC) < 1) {
    swal("Selecciona el Número de la Nota de Crédito que deseas anular", "", "error");
  } else {
    ...
    if (productosCotizados.length) {
      ...
      swal("Debes agregar algún producto a la Nota de Débito!", "", "error");
    }
  }
}
        ...
        if ($("#table-pedido > tbody > tr").length < 1) {
          $("#table-pedido > tbody").append(`
              <tr class="pedido-vacio-msg">
                <th scope="row" colspan="8" class="text-center"><span class="text-muted">Agrega Productos a la Nota de Débito</span></th>
              </tr>
            `);
        }
...
function ClearPedido() {
  ...
  $("#select-nc").val("default").selectpicker("refresh");
  pone_nc();
}
...
async function printCotizacion(dataCotizacion) {
              ...
              <div class="row mt-4 p-0">
                ...
                <div class="col-md-4">
                    <div style="border: 3px solid #FE9A2E !important;border-radius:10px; text-align: center;padding:5px;color:#F7BE81 !important;">
                        ...
                        <h5 style="color:#F7BE81 !important;">Nota de Débito</h5>
        ...
        <tr>
          ...
          <td>${
            descuento && descuento.tipo == "porcentual"
              ? "-" + descuento.valor + "%"
              : descuento && descuento.tipo == "fijo"
              ? "-$" + formatearMonto(descuento.valor)
              : ""
          }</td>
  ...
  getFoliosDisponibles(null, null, 56, numNC, null);
...
function generarNotaDebito(rowid_new, folioND, cafND) {
  const id_cliente = $("#select_cliente").find("option:selected").val();
  ...
  if (hasChanged) {
  ...
  else if (!rowidNC || !rowidNC.length) {
    swal("Selecciona la NC que quieres anular!", "", "error");
  }
  else if (!checkRut(rut)) {
    swal("El RUT ingresado no es válido", "", "error");
  } else if ($(".pedido-vacio-msg").length) {
    ...
    swal("Selecciona el Folio para la Nota de Débito", "", "error");
  } else {
    if (productosCotizados.length) {
      setLoading(true)
      ...
      $.ajax({
        ...
        data: {
          ...
          folioND: folioND,
          cafND: cafND,
          rowid_new: rowid_new ? rowid_new : null
        },
        success: function (x) {
          console.log(x)
          if (x.includes("path")) {
            ...
            window
              .open(
                `verpdf.php?tipo=ND&folio=${folioND}&file=${data.path}`,
                "_blank"
              );
            setLoading(false)
            $("#modal-vistaprevia").modal("hide");
            swal(
              "Generaste la Nota de Débito correctamente!",
            ...
            ClearPedido();
            $("#modal-vistaprevia").modal("hide");
            swal("La Nota de Débito SE ENVIÓ AL SII, pero hubo un error al actualizar los datos en LA BD.", "POR FAVOR COMUNICATE CON SOPORTE ANTES DE CONTINUAR"+x, "error");
          }
          else if (x.includes("ERROR_ENVIO_SII")){
            try {
              const ind1 = x.indexOf("<rowid_new>");
              const ind2 = x.indexOf("</rowid_new>");
              
              if (ind1 >= 0 && ind2 && ind2 >= 0){
                const tmp = x.substring(ind1+11, ind2);
                const rowid_new = tmp.length > 0 && !isNaN(tmp) ? tmp : null;

                if (rowid_new){
                  setLoading(false);
                  reenviarNotaDebito(rowid_new, folioND, cafND)
                }
                else{
                  setLoading(false)
                  ClearPedido();
                  $("#modal-vistaprevia").modal("hide");
                  swal("La Nota de Débito se guardó en el Sistema, pero hubo un error al enviarla al SII.", "Deberás buscar la Nota en el Historial y eliminarla.", "error");
                }
              }
        ...
        error: function (jqXHR, estado, error) {
          swal(
            "Ocurrió un error al guardar la Nota de Débito",
            error.toString(),
            "error"
          );
          setLoading(false)
...
function pone_nc() {
  $("#select-nc").html("").selectpicker("refresh");
  $.ajax({
    beforeSend: function () {
    ...
    type: "POST",
    data: { consulta: "pone_nc" },
    success: function (x) {
      $("#select-nc").html(x).selectpicker("refresh");
...
function reenviarNotaDebito(rowid_new, folioND, cafND) {
  swal(
    `Ocurrió un error al enviar la Nota al SII, ¿Reintentar?`,
    "",
    {
      icon: "error",
      buttons: {
    ...
    switch (value) {
      case "catch":
        generarNotaDebito(rowid_new, folioND, cafND)
