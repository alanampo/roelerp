...
class Sii
{
    ...
    public static function getEstadoDTE($track_id, $retry = null)
    {
        $query = 'QEstadoEnvio2';
        $params = ['TrackId' => $track_id, 'NPagina' => 1];
        $Firma = $GLOBALS["Firma"];
        // solicitar token
        ...
        list($rutCompany, $dvCompany) = explode('-', $GLOBALS["empresa"]["rut"]);
        if (empty($params['rutCompany'])) {
            $params['rutCompany'] = $rutCompany;
            $params['dvCompany'] = $dvCompany;
        }
        ...
        $url = 'https://'.self::$config['servidor'][self::getAmbiente()].'.sii.cl/cgi_dte/UPL/'. $query . '?rutQuery=' . $rutQuery . '&amp;dvQuery=' . $dvQuery. '&amp;token='.$token;
